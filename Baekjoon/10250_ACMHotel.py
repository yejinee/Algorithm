...
손님은 엘리베이터를 타고 이동하는 거리는 신경 쓰지 않는다.
 ...
 예를 들면 102 호 방보다는 301 호 방을 더 선호하는데, 102 호는 거리 2 만큼 걸어야 
 하지만 301 호는 거리 1 만큼만 걸으면 되기 때문이다.
...
[ OUTPUT ] 
프로그램은 표준 출력에 출력한다. 
각 테스트 데이터마다 정확히 한 행을 출력하는데, 내용은 N 번째 
손님에게 배정되어야 하는 방 번호를 출력한다.
...
def RoomNum(H,W,N):
    floor=N%H  #floor
    roomno=N//H+1
    #층수가 0인경우 => 나눠떨어지는 경우 꼭대기층에 배정
    if floor==0:
        floor=H # 꼭대기 층의
        roomno-=1 #roomnum=N//H
    roomnumber=floor*100+roomno
    return roomnumber
