...
dx=[-1,0,0,1]
dy=[0,-1,1,0]
# 최단거리 주어졌을 때, 먹을 물고기 찾음
def find(dist):
    mindist=INF
    for i in range(N):
        for j in range(N):
            if dist[i][j]!=-1 and 0<board[i][j] and board[i][j]<bsize: # 먹는거 가능 
                if dist[i][j]<mindist:
                    x,y=i,j
                    mindist=dist[i][j]
    if mindist==INF:
    ...
    while dq:
        ...
        for k in range(4):
            ...
            if nx<0 or nx>=N or ny<0 or ny>=N:
                continue
            # 지나갈 수 있는 경우(사이즈가 상어보다 작거나 같을때, 이미 지나간곳 아닐때)
            if board[nx][ny]<=bsize and dist[nx][ny]==-1:
                dq.append((nx,ny))
